...
if ($rol == 4) {
		$id=$_GET['id'];
		$ficha=$_GET['ficha'];
		$consulta ="SELECT programa_formacion FROM tab_p_instructor WHERE id_usuario=$id and codigo_ficha=$ficha";
...
if ($rol == 5) {
		$id=$_GET['id'];
		$ficha=$_GET['ficha'];
		$consulta ="SELECT programa_formacion FROM tab_p_aprendiz WHERE id_usuario=$id and codigo_ficha=$ficha";
		$resultado = pg_query($consulta)or die ('Error. Intente de nuevo'. pg_last_error());
		$comprobar= pg_fetch_result($resultado, 0, 0);
